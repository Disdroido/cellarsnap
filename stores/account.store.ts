...
export const useAccountStore = defineStore('account', () => {
  ...
  const changeActiveAccount = async (account_id: string) => {
    const { $client } = useNuxtApp();
  ...
  const acceptPendingMembership = async (membership_id: string) => {
    const { $client } = useNuxtApp();
  ...
  const rejectPendingMembership = async (membership_id: string) => {
    const { $client } = useNuxtApp();
    const { data: membership } =
  ...
  const joinUserToAccountPending = async (account_id: string) => {
    if (!dbUser.value) {
      return;
    }
    const { $client } = useNuxtApp();
